...
function TestPage() {
  ...
  const handleTest = async () => {
    if (!verse || !religion) {
      alert("Please enter both verse and religion!");
      return;
    }
    setLoading(true);
    const output = await processVerse(verse, religion);
    setResult(output);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    const updateAvatarSize = () => {
      ...
      if (width < 480) setAvatarSize("small");
      else if (width < 768) setAvatarSize("medium");
      else setAvatarSize("large");
    };

    updateAvatarSize(); // Initial call
    ...
    <div>
      <h2>🧠 LLM Translation & Validation Test</h2>
        <div style={{ display: "flex", flexDirection: "column", gap: "15px", maxWidth: "400px" }}>
          ...
          <input
            type="text"
            placeholder="Enter religion (e.g., hinduism)"
            value={religion}
        ...
        {loading && <p style={{ marginTop: "20px" }}>Processing with Gemini... ⏳</p>}
